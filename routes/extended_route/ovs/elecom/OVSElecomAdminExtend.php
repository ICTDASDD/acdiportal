...
Route::group(['middleware' => ['auth', 'role:elecom-admin']], function() {
    ...
    //for approve(elecom) 
    Route::get('ovs/elecom/request/editStatus', [ElecomRequestController::class, 'editStatus'])->name('request.edit.status');
    Route::get('ovs/elecom/request/updateStatus', [ElecomRequestController::class, 'updateStatus'])->name('request.update.status');
});
